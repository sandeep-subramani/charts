import { getMinInObject, getMaxInObject } from "../../util";
...
const _getScaleData = (seriesList, axis, axisId, axisType, isMutlipleAxis) => {
  const type = axis.type
    ? axis.type
    : axis.categories && axis.categories.length
    ? "band"
    : "linear";
  ...
  if (isMutlipleAxis) {
    ...
    respectiveSeries = respectiveSeries.length
      ? respectiveSeries
      : seriesList.filter((_, index) => index === axisId);
...
const _extractAxisLineFromAxis = (axis, type) => {
  const {
    axisLine: { color = "#000", width = 1, offset = 0 },
  } = axis;
  return {
    color,
    width,
...
export const convertToBarDrawingJson = ({ meta }) => {
  const drawingJson = {};
